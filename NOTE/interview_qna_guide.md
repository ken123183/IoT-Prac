...
## 7. 壓力測試與效能調優 (k6 & Performance)

### Q11: 為什麼選擇 k6 進行壓力測試？它與傳統的 JMeter 有什麼區別？
**💡 回答要點：開發者體驗 (Developer Experience) 與 技術選型理由。**
*   **k6 的優勢**：k6 使用 JavaScript 撰寫腳本，對開發者非常友善，且底層使用 Go 語言開發，性能極高、內存消耗極低。它非常適合「測試即代碼 (Testing as Code)」的實踐，容易與 CI/CD 整合。
*   **實務應用**：在此專案中，我利用 k6 模擬了 20 位 VU 同時進行租借與歸還操作，驗證了系統在持續高負載下的 100% 成功率。

---

## 8. 前端開發與效能優化 (React & Web Performance)

### Q12: 面對 1,000 顆電池的即時更新，React 頻繁 Re-render 會卡頓嗎？你做了哪些優化？
**💡 回答要點：渲染效能優化。**
*   **組件細粒度控制**：我確保電池列表組件僅更新變動的部分。在後續擴展中，我會考慮引入「虛擬列表 (Virtual List)」技術，只渲染畫面內的可見節點（如 5,000 顆電池以上時）。
*   **數據分流**：我在 UI 上限制了單次渲染的數量（使用 `.slice(0, 50)`），減少了 DOM 節點的渲染負擔。
*   **連線優化**：強制使用 `websocket` 傳輸層，減少了 HTTP 握手造成的瀏覽器主線程阻塞。

---

## 9. 運作維護與容器化 (DevOps & Docker)

### Q13: 在撰寫 Dockerfile 時，你有考慮過鏡像大小優化嗎？
**💡 回答要點：多階段編譯 (Multi-stage Build)。**
是的，我使用了 Multi-stage Build。
1.  **Build Stage**：使用完整 Maven 鏡像進行打包。
2.  **Run Stage**：僅複製編出的 `.jar` 檔到輕量的 `eclipse-temurin:21-jre-alpine`。
這大幅減少了 Production Image 的大小，提升了鏡像拉取速度，並減少了潛在的安全風險。

---

## 10. 安全性與 IoT 協議 (Security & IoT Protocols)

### Q14: 這個專案現在用的是 HTTP POST 發送遙測，為什麼不考慮用 MQTT？
**💡 展現根據場景挑選技術的能力。**
*   **MQTT 優勢**：標頭極小、持續連線、支持 QoS，非常適合網路不穩定的低功耗硬體。
*   **場景分析**：本專案作為「數位孿生」原型，選擇 HTTP 是為了在 Cloud Run (Serverless) 環境下達到最高的自動擴展效率。未來若要接入數萬台真實硬體，我會引進 **GCP IoT Core** 或 **Mosquitto** 作為 MQTT Broker。
